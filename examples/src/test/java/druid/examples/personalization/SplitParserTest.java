...
public class SplitParserTest
{
  ...
  public void testParse(){
    ...
    set.put("attribute","publisher");
    Parser splitParser = new SplitParser();
    splitParser.parse(user,set);
    Assert.assertEquals(user.getDimensionNames().get("publisher"), new Integer(1));
  }

  @Test
  public void testParseIgnoreTimestamp(){
    Map<String,Object> set = Maps.newHashMap();
    set.put("attribute","timestamp");
    Parser splitParser = new SplitParser();
    splitParser.parse(user,set);
    Assert.assertEquals(user.getDimensionNames().get("timestamp"), null);
  }
}
